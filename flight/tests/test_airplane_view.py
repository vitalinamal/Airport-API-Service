...
class AirplaneViewSetTests(APITestCase):
    ...
    def test_retrieve_airplane_authenticated(self):
        self.client.credentials(HTTP_AUTHORIZATION=f"Bearer {self.user_token.access_token}")
        response = self.client.get(reverse("flight:airplanes-detail", kwargs={"pk": self.airplane1.pk}))
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        airplane = Airplane.objects.select_related("airplane_type").get(id=self.airplane1.id)
    ...
    def test_create_airplane_admin(self):
        ...
        response = self.client.post(reverse("flight:airplanes-list"), data, format="json")
    ...
    def test_update_airplane_admin(self):
        ...
        response = self.client.put(reverse("flight:airplanes-detail", kwargs={"pk": self.airplane1.pk}), data)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_partial_update_airplane_admin(self):
        self.client.credentials(HTTP_AUTHORIZATION=f"Bearer {self.admin_token.access_token}")
        data = {
            "name": "Partially Updated Airplane"
        }
        response = self.client.patch(reverse("flight:airplanes-detail", kwargs={"pk": self.airplane1.pk}), data)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.airplane1.refresh_from_db()
    ...
    def test_delete_airplane_admin(self):
        self.client.credentials(HTTP_AUTHORIZATION=f"Bearer {self.admin_token.access_token}")
        response = self.client.delete(reverse("flight:airplanes-detail", kwargs={"pk": self.airplane1.pk}))
