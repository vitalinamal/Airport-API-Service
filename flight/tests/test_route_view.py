...
class RouteViewSetTests(APITestCase):
    ...
    def setUp(self):
        ...
        self.airport3 = Airport.objects.create(name="Airport3", closest_big_city="City3")

        self.route1 = Route.objects.create(source=self.airport1, destination=self.airport2, distance=100)
        self.route2 = Route.objects.create(source=self.airport2, destination=self.airport3, distance=200)
    ...
    def test_retrieve_route_authenticated(self):
        self.client.credentials(HTTP_AUTHORIZATION=f"Bearer {self.user_token.access_token}")
        response = self.client.get(reverse("flight:routs-detail", kwargs={"pk": self.route1.pk}))
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        route = Route.objects.select_related("source", "destination").get(id=self.route1.id)
    ...
    def test_update_route_admin(self):
        ...
        response = self.client.put(reverse("flight:routs-detail", kwargs={"pk": self.route1.pk}), data)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_partial_update_route_admin(self):
        ...
        response = self.client.patch(reverse("flight:routs-detail", kwargs={"pk": self.route1.pk}), data)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.route1.refresh_from_db()
    ...
    def test_delete_route_admin(self):
        self.client.credentials(HTTP_AUTHORIZATION=f"Bearer {self.admin_token.access_token}")
        response = self.client.delete(reverse("flight:routs-detail", kwargs={"pk": self.route1.pk}))
